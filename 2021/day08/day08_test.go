...
func TestCountsNumsInLine(t *testing.T) {
	n := countNumsInLine("be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe")
	assert.Equal(t, 2, n)
}
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce`
	assert.Equal(t, 26, countNums(strings.NewReader(lines)))
}
